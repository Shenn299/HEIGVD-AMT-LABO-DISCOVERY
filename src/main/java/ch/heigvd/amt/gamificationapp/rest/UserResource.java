...
/**
 * Entry point for all HTTP requests which treat user resources. Methods set: - {@link #getUsers(java.lang.String)}
 *    - {@link #createUser(ch.heigvd.amt.gamificationapp.rest.dto.UserDTO)}
 *    - {@link #getUser(long)}
 *    - {@link #deleteUser(long)}
 *    - {@link #patchUser(long, ch.heigvd.amt.gamificationapp.rest.dto.UserDTO)}
 *
...
public class UserResource {
     ...
     *         doesn't work. <code>CONFLICT</code>, if username already exists
     *         in database. <code>CREATED</code>, if new <code>User</code> could
     *         be created.
     */
    @POST
     ...
     *         processing SQL request. <code>CONFLICT</code>, if username
     *         already exists in database. <code>NOT_FOUND</code>, if id doesn't
     *         exist. <code>OK</code>, if new <code>User</code> could be
     *         updated.
     */
    @Path("{id}")
    @PATCH
